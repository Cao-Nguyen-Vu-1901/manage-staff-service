...
public class RewardDisciplineServiceImp implements IRewardDisciplineService {
    ...
    public List<RewardDisciplineResponse> findAllById(List<String> ids) {
        return rewardDisciplineRepository.findAllById(ids).stream()
                .map(rewardDisciplineMapper::toRewardDisciplineResponse)
                .collect(Collectors.toList());
    }
    ...
    public List<RewardDisciplineResponse> findAllByNameLike(String name) {
        return rewardDisciplineRepository.findAllByNameLike("%" + name + "%").stream()
                .map(rewardDisciplineMapper::toRewardDisciplineResponse)
                .collect(Collectors.toList());
    }

    @Override
    public Page<RewardDisciplineResponse> paging(
            String column, String value, int currentPage, int pageSize, String orderBy, String sortBy) {
        return rewardDisciplineDAO
    ...
    public RewardDisciplineResponse findById(String id) {
        return rewardDisciplineMapper.toRewardDisciplineResponse(rewardDisciplineRepository
                .findById(id)
